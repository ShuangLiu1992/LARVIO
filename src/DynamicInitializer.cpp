...
// The core method comes from VINS-MONO
...
void DynamicInitializer::processIMU(const ImuData &imu_msg) {
  ...
  if (!pre_integrations[frame_count]) // qxc: into this if only at the first
                                      // time
  {
    pre_integrations[frame_count].reset(
...
void DynamicInitializer::processImage(MonoCameraMeasurementPtr img_msg) {
  ...
  tmp_pre_integration.reset(new IntegrationBase{acc_0, gyr_0, Bas[frame_count],
                                                Bgs[frame_count], acc_n, acc_w,
                                                gyr_n, gyr_w});
...
bool DynamicInitializer::initialStructure() {
  ...
  GlobalSFM sfm;
  if (!sfm.construct(frame_count + 1, Q.data(), T.data(), l, relative_R, relative_T, sfm_f,
                     sfm_tracked_points)) {
...
bool DynamicInitializer::visualInitialAlign() {
  ...
  // Set rotation
  Matrix3d R_c02w = Quaterniond::FromTwoVectors(gravity_c0, -gravity_world)
                        .toRotationMatrix();
...
bool DynamicInitializer::relativePose(Matrix3d &relative_R,
                                      Vector3d &relative_T, int &l) {
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    if (corres.size() > 20) {
      ...
      if (average_parallax * 460 > 30 &&
        ...
        l = i;
        // printf("average_parallax %f choose l %d and newest frame to
...
void DynamicInitializer::assignInitialState(
    std::vector<ImuData> &imu_msg_buffer, Eigen::Vector3d &m_gyro_old,
    Eigen::Vector3d &m_acc_old, IMUState &imu_state) {
  if (!bInit) {
