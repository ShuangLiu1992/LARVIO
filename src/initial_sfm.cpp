...
bool GlobalSFM::solveFrameByPnP(Matrix3d &R_initial, Vector3d &P_initial, int i,
  ...
  if (int(pts_2_vector.size()) < 15) {
    printf("unstable features tracking, please slowly move you device! or just "
           "put in on the table!\n\n");
...
bool GlobalSFM::construct(int frame_num, Quaterniond *q, Vector3d *T, int l,
  ...
  // rotate to cam frame
  std::vector<Matrix3d> c_Rotation(frame_num);
  std::vector<Vector3d> c_Translation(frame_num);
  std::vector<Quaterniond> c_Quat(frame_num);
  ...
  */
  ...
  for (int i = 0; i < feature_num; i++) {
    ...
    for (int j = 0; j < int(sfm_f[i].observation.size()); j++) {
      ...
      problem.AddResidualBlock(cost_function, NULL, c_rotation[l].data(),
                               c_translation[l].data(), sfm_f[i].position);
    }
  }
  ...
  for (int i = 0; i < frame_num; i++) {
    T[i] =
        ...
        (q[i] *
         Vector3d(
             c_translation[i][0], c_translation[i][1],
             c_translation[i][2])); // qxc: 优化出来的是t_cr_c，需要转成t_rc_r
